    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def reset(
        ...
        *,
        ...
        # Create Lander body
        initial_y = self.np_random.uniform(10.0, 11.0)   
        initial_x = self.np_random.uniform(8.5, 11.5)   
    ...
    # ---------- Safety-set helpers ----------
    def _in_S_flight(self, s):

        x,y,vx,vy,th,om,c1,c2 = s
        return ( (abs(x) <= 0.25) and
                 (y < 1.3) and
                 (abs(th) <= 0.4) and (abs(om) <= 0.2) )

    def _in_S_touch(self, s):
        x,y,vx,vy,th,om,c1,c2 = s
        touched = (c1 == 1.0 or c2 == 1.0)
        return ( touched and
                 (abs(x) <= 0.2) and
                 (abs(vx) <= 0.1) and (vy >= -0.1) and
                 (abs(th) <= 0.2) and (abs(om) <= 0.2) )
    # ---------- End of Safety-set helpers ----------            
    ...
    def step(self, action):
        ...
        vel_threshold = 0.5
        vel_current = np.sqrt(state[2] * state[2] + state[3] * state[3])

        angle_threshold = np.pi / 4   # 45 degrees in radians
        angle_current = abs(state[4])

        safety_signal = 1.0 if (
            (vel_current <= vel_threshold) and
            (angle_current <= angle_threshold) and
            (abs(state[0]) <= 0.4)
        ) else 0.0
        
        self.safety_signal = safety_signal
        
        # print("vel:", vel_current)
        # print("angle:", angle_current)
        # print("safe:", safety_signal)
        ...
        return np.array(state, dtype=np.float32), reward, False, False, info
